...
windows = np.arange(0.9, 0.5, -0.05)          # [0.9, 0.8, 0.7, 0.6]
steps   = np.arange(0.03, 0.599, 0.03)          # [0.03, 0.06, ..., 0.57]
slice_parameters = [(w, s) for w in windows for s in steps if s < w]

#Step 3: Start a loop
for window,step in slice_parameters:
    # Step 4: Slice the data
    X_train_sliced, y_train_sliced = expand_training_data(X_train_all[:20], y_train_all[:20], round(window*len(X_train_all)), round(step*len(X_train_all)))
    ...
    for Xtrial, Ytrial in zip(X_test, y_test):
        newX, newY = expand_training_data([Xtrial], [Ytrial], round(window*len(X_train_all)), round(step*len(X_train_all)))
...
x_labels = [f'Win: {round(w, 2)}, Step: {round(s, 2)}' for w, s in slice_parameters]
